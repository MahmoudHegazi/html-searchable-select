const HTMLSearchableSelect = (function() {
    ...
    class HTMLSearchableSelect {
        ...
        addEvents() {
            ...
            if ($(this.searchSelector).length) {
                $(this.searchSelector).on('input.hss', () => {
                    // old version  (try ommt search and comment customSearchInputEvent see performance diff this last performance commit--)
                    this.search();
                    // new jquery
                    //this.customSearchInputEvent();
                });
            }
        ...
        customSearchInputEvent() { /* old code deadcode */
            // update performance  (instead run search on every input event, no wait until input value stay same for period of time and then call 1 search at end of writting (wait for user complete writing then trigger input (no action required like change smallest thing can said benfit)))
            if (this.customInput){
                clearTimeout(this.customInput);
                this.customInput = null;
            }

            const val = $(this.searchSelector).val();
            if (val == this.customInputOldVal) { /* technique checker */
                this.customInputOldVal = null;
                this.search();
            } else {
                this.customInputOldVal = val;
                this.customInput = setTimeout(()=>{this.customSearchInputEvent();/* arrow and this */}, this.customInputWait);
            }
        }
